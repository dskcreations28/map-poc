...
export class IndiaChoropheltComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  @ViewChild('cityMapContainer') cityMapContainer!: ElementRef;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['viewMode']) {
      if (this.viewMode === 'city') {
        ...
        // Show empty state for city view
        const container = this.cityMapContainer.nativeElement;
        container.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#777;font-size:1.2rem;font-weight:500;">City view — No Map</div>';
      } else {
        this.renderDistrictDynamic();
      }
    }
  }
  ...
  private initDistrictMap(): void {
    if (this.viewMode === 'district') {
      this.map = L.map(this.districtMapContainer.nativeElement, {
  ...
  private renderDistrictDynamic(): void {
    const container = this.viewMode === 'district'
      ? this.districtMapContainer.nativeElement
      : this.cityMapContainer.nativeElement;
    container.innerHTML = '';
    if (!this.map) {
      this.initDistrictMap();
    }
